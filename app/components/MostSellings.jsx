...
export default function MostSellings() {
  const [products, setProducts] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/mostsellings', {
          cache: 'no-store',
        });
        if (!res.ok) {
      ...
      {products.length !== 0 ? (
        <section className="montserrat-font z-10 w-full px-4 py-10 mt-4">
          <div className="montserrat-font flex justify-between items-center mb-6">
            <h2 className="montserrat-font text-3xl font-bold text-gray-900">Most Sellings</h2>
          </div>
          <div className="montserrat-font flex justify-start p-1 overflow-x-auto gap-6 scrollbar-hide">
            {Array.isArray(products) &&
              products.map((product, idx) => (
                ...
                >
                  <div className="relative montserrat-font w-full h-[160px] mb-4 overflow-hidden">
                    <Image
                    ...
                    />
                    <div className="montserrat-font absolute top-0 right-0 bg-red-500 text-white p-2 animate-pulse">
                  ...
                  <p className="montserrat-font text-l text-gray-700 italic">
                    Price:{' '}
                    <span className="montserrat-font text-xl text-green-600">
          ...
          <div className="montserrat-font flex justify-between items-center mb-6">
            <Link
              href="/product"
              className="montserrat-font text-black underline font-medium hover:text-amber-600 pt-2"
            >
              View all products
            </Link>
          </div>
